...
function doRegister() {
  ...
  fbAuth
    ...
    .then(function () {
      ...
      ["rFN", "rLN", "rEmail", "rPass"].forEach(function (id) {
        if (g(id)) g(id).value = "";
      });
      switchTab("login");
      g("liEmail").value = em;
...
function updateNav() {
  ...
  s("profAv", ini);
  s("profName", currentUser.firstName + " " + currentUser.lastName);
  s("profEmail", currentUser.email);
}

function toggleMenu() {
  g("userMenu").classList.toggle("open");
}
function closeMenu() {
  g("userMenu").classList.remove("open");
}
document.addEventListener("click", function (e) {
  if (!e.target.closest(".avatar-wrap")) closeMenu();
});
...
function applyRoleUI() {
  ...
  document.querySelectorAll(".alumni-only").forEach(function (el) {
    el.style.display = role === "alumni" || role === "admin" ? "" : "none";
  });
  document.querySelectorAll(".student-only").forEach(function (el) {
    el.style.display = role === "student" || role === "admin" ? "" : "none";
  });
  var pb = g("postJobBtn");
  if (pb)
    pb.style.display =
      role === "admin" || role === "alumni" || role === "recruiter"
        ? ""
        : "none";
}
...
function showView(name) {
  ...
  var lk = document.querySelector('.nav-link[data-view="' + name + '"]');
...
function loadAlumni() {
  fbDB
    ...
    .onSnapshot(
      ...
      function () {
        showToast("Could not load alumni", "error");
      },
    );
}
...
function renderActivity() {
  ...
  el.innerHTML = activityLog
    .map(function (a) {
      return (
        '<div class="act-item"><div class="act-ico">' +
        a.ico +
...
function renderAlumni() {
  ...
  if (!page.length) {
    el.innerHTML =
      '<p class="empty-msg">No alumni found. ' +
      (isAdmin ? 'Click "+ Add Alumni" to add the first one.' : "") +
      "</p>";
  ...
  el.innerHTML = page
    .map(function (a) {
      ...
      var delBtn = isAdmin
        ? '<button class="icon-btn del-btn" title="Delete" onclick="deleteAlumni(\'' +
          a.id +
          "','" +
      ...
      return (
        ...
        '<div class="ac-top">' +
        '<div class="ac-av" style="background:' +
        col +
        '">' +
        ini +
        "</div>" +
        '<div class="ac-info"><div class="ac-name">' +
        a.firstName +
        " " +
        a.lastName +
        "</div>" +
...
function addAlumni() {
  ...
  if (!batch) {
    showToast("Enter Graduation Year", "error");
    return;
  }
  ...
  var data = {
    ...
    desig: v("aDesig"),
    city: v("aCity"),
    industry: v("aIndustry"),
    linkedin: v("aLinkedin"),
    status: "active",
...
function deleteAlumni(id, name) {
  confirmDelete(
    'Delete alumni "' + name + '"? This cannot be undone.',
    function () {
      fbDB
        .collection("alumni")
        .doc(id)
        ...
        .then(function () {
          ...
          pushActivity("🗑️", "Alumni deleted: " + name);
        })
        .catch(function (e) {
          showToast("Error: " + e.message, "error");
        });
...
function renderEvents() {
  ...
  if (!list.length) {
    el.innerHTML =
      '<p class="empty-msg">No events yet.' +
      (isAdmin ? " Create the first event!" : "") +
      "</p>";
    return;
  }
  ...
  el.innerHTML = list
    .map(function (ev) {
      ...
      var delBtn = isAdmin
        ? '<button class="icon-btn del-btn" title="Delete event" onclick="deleteEvent(\'' +
          ev.id +
          "','" +
          (ev.title || "").replace(/'/g, "") +
          "')\">🗑️</button>"
      ...
      var rsvpBtn =
        ev.status !== "past"
          ? '<button class="btn btn-sm btn-primary" onclick="rsvpEvent(\'' +
            ev.id +
            "')\" " +
            (ev.registrations >= ev.maxAttendees ? "disabled" : "") +
            ">" +
            (ev.registrations >= ev.maxAttendees ? "Full" : "RSVP") +
            "</button>"
...
function createEventRecord() {
  ...
  var data = {
    title: title,
    type: type,
    status: v("eStatus") || "upcoming",
    date: date,
    time: v("eTime"),
    venue: v("eVenue"),
    maxAttendees: parseInt(v("eMax")) || 100,
    registrations: 0,
    fee: parseInt(v("eFee")) || 0,
    desc: v("eDesc"),
    emoji: emojis[type] || "📅",
  ...
  fbDB
    ...
    .then(function () {
      ...
      ["eTitle", "eDate", "eTime", "eVenue", "eMax", "eDesc"].forEach(
        function (id) {
          if (g(id)) g(id).value = "";
        },
      );
  ...
  confirmDelete('Delete event "' + title + '"?', function () {
    fbDB
      ...
      .then(function () {
        ...
        pushActivity("🗑️", "Event deleted: " + title);
      })
      .catch(function (e) {
        showToast("Error: " + e.message, "error");
      });
...
function rsvpEvent(id) {
  var ev = eventsData.find(function (e) {
    return e.id === id;
  });
  if (!ev) return;
  if (ev.registrations >= ev.maxAttendees) {
  ...
  fbDB
    .collection("events")
    .doc(id)
    .update({ registrations: (ev.registrations || 0) + 1 })
    .then(function () {
      showToast('RSVP confirmed for "' + ev.title + '"! 🎉', "success");
      pushActivity("📅", "RSVP for " + ev.title);
    })
    .catch(function (e) {
      showToast("Error: " + e.message, "error");
    });
...
function renderMentors() {
  ...
  var el = g("mentorsGrid");
  if (!el) return;
  var isAdmin = currentUser && currentUser.role === "admin";

  if (!list.length) {
    el.innerHTML =
      '<p class="empty-msg">No mentors yet.' +
      (isAdmin ? " Add the first mentor!" : "") +
      "</p>";
    return;
  }
  ...
  el.innerHTML = list
    .map(function (m) {
      ...
      var delBtn = isAdmin
        ? '<button class="icon-btn del-btn" title="Delete mentor" onclick="deleteMentor(\'' +
          m.id +
          "','" +
          (m.name || "").replace(/'/g, "") +
          "')\">🗑️</button>"
      ...
      return (
        ...
        '<div class="mc-top">' +
        '<div class="mc-av" style="background:' +
        col +
        '">' +
        ini +
        "</div>" +
        '<div class="mc-info"><div class="mc-name">' +
        (m.name || "—") +
        '</div><div class="mc-role">' +
        (m.role || "—") +
        " · " +
        (m.company || "—") +
        "</div></div>" +
        ...
        (m.domains || [])
          .map(function (d) {
            return '<span class="domain-chip">' + d + "</span>";
          })
          .join("") +
        "</div>" +
        '<div class="mc-footer">' +
        ...
        "</span>" +
        '<button class="btn btn-sm btn-primary" onclick="requestMentorByName(\'' +
        (m.name || "").replace(/'/g, "") +
        "')\">Request</button>" +
        "</div></div>"
      );
...
function addMentor() {
  ...
  if (!company) {
    showToast("Enter Company", "error");
    return;
  }
  if (!role) {
    showToast("Enter Role", "error");
    return;
  }
  if (!domains) {
  ...
  confirmDelete('Delete mentor "' + name + '"?', function () {
    fbDB
      ...
      .then(function () {
        ...
        pushActivity("🗑️", "Mentor deleted: " + name);
      })
      .catch(function (e) {
        showToast("Error: " + e.message, "error");
      });
...
function renderJobs() {
  ...
  var typeClass = {
    "Full-time": "tj-full",
    Internship: "tj-intern",
    "Part-time": "tj-part",
    Contract: "tj-contract",
  };

  if (!list.length) {
    el.innerHTML = '<p class="empty-msg">No jobs posted yet.</p>';
    return;
  }
  ...
  el.innerHTML = list
    .map(function (j) {
      ...
      var delBtn = canDel
        ? '<button class="icon-btn del-btn" title="Delete" onclick="deleteJob(\'' +
          j.id +
          "','" +
          (j.title || "").replace(/'/g, "") +
          "')\">🗑️</button>"
      ...
      return (
        '<div class="job-card">' +
        '<div class="jc-logo" style="background:' +
        col +
        '">' +
        ini +
        "</div>" +
        '<div class="jc-body">' +
        '<div class="jc-top"><div><div class="jc-title">' +
        (j.title || "—") +
        '</div><div class="jc-company">' +
        (j.company || "—") +
        "</div></div>" +
        '<div style="display:flex;gap:.5rem;align-items:center"><span class="jtype ' +
        (typeClass[j.type] || "") +
        '">' +
        (j.type || "—") +
        "</span>" +
        ...
        "</div>" +
        '<div class="jc-footer">' +
        '<span style="font-size:.78rem;color:var(--muted)">Posted ' +
        (j.postedBy || "—") +
        "</span>" +
        '<button class="btn btn-sm btn-primary" onclick="applyJob(\'' +
        (j.title || "").replace(/'/g, "") +
        "','" +
        (j.company || "").replace(/'/g, "") +
        "')\">Apply Now</button>" +
...
function postJob() {
  var title = v("jTitle"),
    company = v("jCompany");
  if (!title) {
    showToast("Enter Job Title", "error");
    return;
  }
  if (!company) {
  ...
  var data = {
    title: title,
    company: company,
    type: v("jType") || "Full-time",
    location: v("jLocation"),
    salary: v("jSalary"),
    deadline: v("jDeadline"),
    desc: v("jDesc"),
    applyLink: v("jLink"),
    postedBy: currentUser ? currentUser.email : "guest",
    createdAt: ts(),
  };
  ...
  confirmDelete('Delete job "' + title + '"?', function () {
    fbDB
      ...
      .then(function () {
        ...
        pushActivity("🗑️", "Job deleted: " + title);
      })
      .catch(function (e) {
        showToast("Error: " + e.message, "error");
      });
...
function applyJob(title, company) {
  fbDB
    ...
    .add({
      jobTitle: title,
      company: company,
      appliedBy: currentUser ? currentUser.email : "guest",
      createdAt: ts(),
    })
    .then(function () {
      showToast(
...
function renderCampaigns() {
  var el = g("campaignsGrid");
  if (!el) return;
  var isAdmin = currentUser && currentUser.role === "admin";

  if (!campaignsData.length) {
    el.innerHTML =
      '<p class="empty-msg">No campaigns yet.' +
      (isAdmin ? " Create one!" : "") +
      "</p>";
    return;
  }

  el.innerHTML = campaignsData
    .map(function (c) {
      var pct = Math.round(
        Math.min(((c.raised || 0) / (c.target || 1)) * 100, 100),
      );
      var delBtn = isAdmin
        ? '<button class="icon-btn del-btn" title="Delete campaign" onclick="deleteCampaign(\'' +
          c.id +
          "','" +
          (c.title || "").replace(/'/g, "") +
          "')\">🗑️</button>"
      ...
      return (
        ...
        delBtn +
        "</div>" +
        '<div class="cc-amt"><span class="cc-raised">₹' +
        (c.raised || 0).toLocaleString() +
        '</span><span class="cc-goal"> of ₹' +
        (c.target || 0).toLocaleString() +
        "</span></div>" +
        '<div class="cc-bar"><div class="cc-fill" style="width:' +
        pct +
        '%"></div></div>' +
        '<div class="cc-footer"><span>👥 ' +
        (c.donors || 0) +
        " donors · " +
        pct +
        "%</span>" +
...
function createCampaign() {
  ...
  if (!title) {
    showToast("Enter Campaign Title", "error");
    return;
  }
  if (!target || target < 1) {
  ...
  fbDB
    ...
    .then(function () {
      ...
      ["cTitle", "cTarget", "cEndDate", "cIcon", "cDesc"].forEach(
        function (id) {
          if (g(id)) g(id).value = "";
        },
      );
...
function deleteCampaign(id, title) {
  confirmDelete(
    'Delete campaign "' + title + '"? All related data remains.',
    function () {
      fbDB
        .collection("campaigns")
        .doc(id)
        ...
        .then(function () {
          ...
          pushActivity("🗑️", "Campaign deleted: " + title);
        })
        .catch(function (e) {
          showToast("Error: " + e.message, "error");
        });
...
function renderDonations() {
  var el = g("donorsTable");
  if (!el) return;
  var isAdmin = currentUser && currentUser.role === "admin";

  if (!donationsData.length) {
    el.innerHTML =
      '<tr><td colspan="6" style="text-align:center;color:var(--muted);padding:1.5rem">No donations yet</td></tr>';
    return;
  }

  el.innerHTML = donationsData
    .map(function (d) {
      var delBtn = isAdmin
        ? '<td class="admin-only"><button class="icon-btn del-btn" onclick="deleteDonation(\'' +
          d.id +
          "','" +
          (d.name || "").replace(/'/g, "") +
          "')\">🗑️</button></td>"
...
function makeDonation() {
  ...
  if (!campaignId || campaignId === "No campaigns yet") {
  ...
  var camp = campaignsData.find(function (c) {
  ...
  batch
    ...
    .then(function () {
      ...
      showToast(
        "Thank you for donating ₹" + amount.toLocaleString() + "! 💰",
        "success",
      );
      pushActivity(
        "💰",
...
function sendBroadcast() {
  ...
  fbDB
    ...
    .add({
      ...
      message: msg,
      recipients: v("bRecip"),
      sentBy: currentUser ? currentUser.email : "admin",
      sentAt: ts(),
    })
    .then(function () {
...
function exportCSV() {
  if (!alumniData.length) {
    showToast("No alumni data to export", "warning");
    return;
  }
  var hdrs = [
    "First Name",
...
function loadProfile() {
  if (!currentUser) return;
  var fs = function (id, val) {
    if (g(id)) g(id).value = val || "";
  };
  fs("sFirstName", currentUser.firstName);
  fs("sLastName", currentUser.lastName);
  fs("sEmail", currentUser.email);
  fs("sPhone", currentUser.phone);
  fs("sCompany", currentUser.company);
  fs("sDesig", currentUser.desig);
  fs("sCity", currentUser.city);
  fs("sLinkedin", currentUser.linkedin);
...
function saveProfile() {
  ...
  currentUser.phone = v("sPhone");
  currentUser.company = v("sCompany");
  currentUser.desig = v("sDesig");
  currentUser.city = v("sCity");
  currentUser.linkedin = v("sLinkedin");
  currentUser.bio = v("sBio");
  ...
  if (fbUser && np) {
    fbUser
      ...
      .then(function () {
        ...
        ["currPass", "newPass", "confPass"].forEach(function (id) {
          if (g(id)) g(id).value = "";
        });
      })
      .catch(function (e) {
...
function saveInstitution() {
  var name = v("instName");
  if (!name) {
    showToast("Institution name cannot be empty", "error");
    return;
  }
  fbDB
    .collection("settings")
    ...
    .then(function () {
      showToast("Institution settings saved! 🏛️", "success");
    })
    .catch(function () {
...
function showStab(tab, el) {
  ...
  document.querySelectorAll(".snav").forEach(function (l) {
    l.classList.remove("active");
  });
  var t = g("st-" + tab);
...
function renderNotifications() {
  ...
  if (!notifications.length) {
    el.innerHTML =
      '<p class="empty-msg" style="padding:2rem">No notifications yet</p>';
    return;
  }
